...
//    private int effect;//图片切换的动画效果
    ...
    public CarouselView(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,R.styleable.CarouselView,0,defStyle);
//        mDrawable = a.getDrawable(R.styleable.CarouselView_defalut_image);
        mDefaultImageId = a.getResourceId(R.styleable.CarouselView_defalut_image,0);
        a.recycle();
        initUI(context);
        if (!(imageUris.size() <= 0))
        {
    ...
    public void setImageUris(List<String> imageuris) {
        if (imageuris.size() <= 0)// 如果得到的图片张数为0，则增加一张默认的图片
        {
            if(mDefaultImageId == 0) {
                throw new IllegalArgumentException("If has no image url added , then you should set a default image");
            }
            imageUris.add("drawable://" + mDefaultImageId);
        }
        else
        {
        ...
        } else
        {
            ...
            try
            {
                ...
                FixedSpeedScroller scroller = new FixedSpeedScroller(
                        mViewPager.getContext(), new LinearInterpolator());
